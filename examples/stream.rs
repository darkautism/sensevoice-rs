...
use std::thread;
use futures::stream::StreamExt; // For .next() in infer_stream if used, but here we pass Reader?
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stream = reader_to_stream(delayed_reader);
    let mut stream_out = svs.infer_stream(stream);
